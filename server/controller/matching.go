...
func FindMatching(c *gin.Context) {
	...
	db.Sess.Select("*").From("friends").Where("follow_id = ?", cal.UserID).Load(&conn)
	...
	for _, t := range cal.FreeDate {
		...
		end_date, _ := time.Parse("2006-01-02", t.EndDate)
		for _, con := range conn {
			...
			end_date_tag, _ := time.Parse("2006-01-02", free_d.EndDate)
